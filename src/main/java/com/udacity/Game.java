...
public class Game {
    ...
    public void nextTurn(){
        ...
        if(!twoPlayer){
            ...
            }while(grid[ai_i][ai_j] != '-'); //keep tryin if this spot was taken
            //update grid with new play, compugter is always o
            grid[ai_i][ai_j] = 'o';
            //update free spots
            freeSpots--;
        }
...
//     * @param grid 2D array of characters representing the game board
    ...
    public String checkGameWinner(char[][] grid){
        ...
        char horizontalWinner = horizontalCheck();
        char verticalWinner = verticalCheck();
        char diagonalWinner = diagonalCheck();
        boolean isTie = checkForTie(result);


            if(horizontalWinner == 'x' || verticalWinner == 'x' || diagonalWinner == 'x'){
                result = "X wins";
            }else if(horizontalWinner == 'o' || verticalWinner == 'o' || diagonalWinner == 'o'){
                result = "O wins";
            } else if(isTie){
                result = "Tie";
            }
...
//                System.out.println(result);
    ...
    /**
     * Checks the horizontal side of the Grid
     * Based on the horizontal check it returns the winning char
     * If 'X' wins it returns it
     * IF 'O' wins, it returns it
     * @return char indicating the winner 'X' or 'O' char
     */
    public char horizontalCheck(){
        char[][] grid = this.grid;

        char winner;
        /* Check for horizontal wins with 'X' */
    ...
    /**
     * Checks the Vertical side of the Grid
     * Based on the horizontal check it returns the winning char
     * If 'X' wins it returns it
     * IF 'O' wins, it returns it
     * @return char indicating the winner 'X' or 'O' char
     */
    public char verticalCheck(){
        char[][] grid = this.grid;
    ...
    /**
     * Checks the diagonal side of the Grid
     * Based on the horizontal check it returns the winning char
     * If 'X' wins it returns it
     * IF 'O' wins, it returns it
     * @return char indicating the winner 'X' or 'O' char
    ...
    /**
     * Checks whether there's a tie
     * When there's is no line of 3 X's or 3 O's
     * @return String indicating there's a tie
     */
    public boolean checkForTie(String result){
        return !result.equals("X wins") && !result.equals("O wins") && freeSpots == 0;
    }
